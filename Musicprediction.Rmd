...
In order to limit the processing times, only `r percent`% of the data are used during the report writing process. As soon as the project matures, calculation times will be assessed and this limit will be adapted if possible.

With 3-fold crossvalidation and 5% of the data, the script completed in under 2 minutes on a 2017 iMac.
...
The artist name would be a subjective parameter with high predictive power (many artists tend to be in some genre for most of their career). However, being a factor with over a thousand levels, this feature was found to increase computation times beyond practical limits, so it is excluded from the models.
...
As discussed above, the seven following predictors are used as 'objective parameters':
...
A knn-model will be fitted using the six objective predictors above. In order to somewhat limit computation times, `r n_fold`-fold cross-validation will be used to speed up the model fitting, and inital resolutin will be low.
...
In a second step, k-values approaching this limit at k=500 will be examined with better resolution.
...
The performance of the knn model was the same, whether 6 or 13 predictors were included. Both models performed best with a k just below a threshold of 500, where the modelling function raised an error because 'too many ties' affected the voting process.

Both random forests perfomed best when six variables were sampled for each tree.
